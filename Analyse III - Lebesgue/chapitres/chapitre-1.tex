...
\prf{Soit $\gamma$ et $V$ comme donnée dans l'énoncé, et $\bm f \in C^1$, alors par régularité de $\gamma$, on a que 
	...
	\]
	Maintenant qu'on a un recouvrement, on utilise la proposition précédente avec $K = \overline V$, et $(U_i, V_i)$, $i \in \llbracket 1, N+1 \rrbracket$. Ensuite on remplce $\bm f$ par $\varphi_i \bm f \in C^1_0(V_i)$.
}
